...
> **📍 当前状态（2026-09-08）**：**一轮全站学习已告一段落**。主轨（夜间定时）与闲时轨（暂存补学）双轨收口：账本 `AI相关/_meta/ws_scan_progress.json` 的 `completeness.done=true`、`--top` 空、候选池全部处置（已学习 522 + 无 Lua 标记 1239），例题集 `数据库\Lua\Lua示例\` 01~09 全册。本规程与 `AI相关\_scripts\` 工具链、SteamCMD 本体（`<WS>\_tools\steamcmd`）**原样保留**，后续可随时重启：工坊出现新作品时重新执行本规程，价值轨会自动排出新候选；双轨并行与晨间合并机制见 `闲时任务提示词.md`。「已学习 / 无 Lua」判定账本在 `AI相关/_meta/`，重启前先读它防重学。
>
> **🧹 同日仓库瘦身**：`_提取暂存\`、`_r5src\`、`_r5\` 中间产物已打包为
> `<WS>\_work\归档_工坊提取中间产物_2026-09-08.zip`（33 247 文件 / 36 MB，含完整性校验）后删除散文件；
> 需要复核某段 Lua 时解包该 zip。重启学习时 `_提取暂存\` 会被提取器自动重建，无需恢复旧数据。
...
5. ~~**保留** `_提取暂存/`~~ **2026-09-08 已归档瘦身**：打包为 `<WS>\_work\归档_工坊提取中间产物_2026-09-08.zip` 后删除散文件（见头部状态注记）。
...
- 🔴 **动笔前必须先 grep 示例集核对（2026-09-04 两次踩坑后固化为硬规则）**：`census` / `pick_lua_candidates` 的**打分只看手法稀有度，看不出「这是不是已有例题的搬运」**。已连续两轮出现「高分候选整段与既有章节逐行相同」（第 9 轮：3794688360 的 vec3 闭包库 ≈ `04` §11、定宽协议 ≈ `07` §9；第 10 轮：3795434013 的声纳 ping 往返计时 ≈ `03` §11、3×5 ROM 字体 ≈ `01` §5 / `03` §18）。**流程**：选定候选 → 在 `<WS>\数据库\Lua\Lua示例\` 全目录 grep 该手法的特征函数名/关键表达式/来源 id → 命中已有章节则**跳过**（除非它有旧例没有的**新**手法，此时写成「补充/改进」并显式关联旧章节）。不执行这一步必然写出重复例题，白烧一整轮注意力。
...
- 全部处理完（或用户叫停）后：
  ...
  - `_提取暂存\` 已于 2026-09-08 归档为 `_work\归档_工坊提取中间产物_2026-09-08.zip` 并删除散文件；复核需求解包该 zip，重启学习时目录自动重建。
...
| `AI相关/_scripts/extract_workshop_lua.py` | 附录 A 提取器 + 统计，额外输出 `_manifest.json`（id → 块数/字符数/是否压缩）。支持 `--src DIR`（可重复，多源依次扫描）、`--out DIR`、**`--only id,id,…`（2026-09-04 起，只提取指定 id，配合 `--out` 实现批次隔离）**；`--only` 时额外打印「无 Lua 的 id」，可直接喂 `ws_rank.py --no-lua` | 默认仍是订阅目录 + `_提取暂存` |
| `AI相关/_scripts/fetch_meta.py` | 步骤 2：Steam Web API 批量取元数据 → `_meta/items.json` | 41/41 成功率，优于抓 HTML |
| `AI相关/_scripts/census_lua.py` | **省注意力关键**：对全部 Lua 按 SW 专属 API 加权打分、打功能标签，输出 `_census.json`。**支持 `--src DIR`**（只普查某个批次）、`--topn N`（控制榜单行数）、`--quiet`（只写 JSON 不打印） | 别通读，先普查再挑 |
| `AI相关/_scripts/dedup_pick.py` | 按内容 MD5 去重 + 按类别给推荐精读清单，输出 `_unique.json`。**2026-09-04 起支持 `--src DIR` / `--topn N` / `--notopic`**（批次隔离 + 省上下文） | 2308 块 → 1009 唯一（重复率 56%）；不带参数仍是全量主语料 |
| **`AI相关/_scripts/dedup_lua_batch.py`** | **批次版去重**：`--` 传逗号分隔 id 或用位置参数传 id 列表，只对本批去重并打印选题清单 | 口径同 `dedup_pick`（剥头部两行后 MD5） |
| **`AI相关/_scripts/diff_lua_batch.py`** | 🔑 **本批 vs 主语料哈希差分**：剔除「主语料里已经有了」的脚本，只留全新代码。**2026-09-04 起把重复项再分成「归属作品已学习（可安全跳过）」与「归属作品未学习（⚠ 必须补学）」两类** | **每批必跑**，见下方「批内去重 ≠ 全语料去重」与「重复 ≠ 可跳过」 |
| `AI相关/_scripts/dump_lua.py` | 按文件名批量打印某个目录下的脚本（省一次一个 Read）。支持 `--dir DIR`、`--maxc N`（单文件截断，防上下文爆）、`--head N` | `dump_lua.py --dir _b1 --maxc 4000 a.lua b.lua` |
| **`AI相关/_scripts/peek_lua.py`** | 🆕 **（2026-09-04）极低上下文速览 + 字符预算装箱**：每文件 1 行摘要；`--budget N` 按分数贪心挑出「预算内最多文件」（单文件默认不超预算 1/5，`--maxfile` 可改）；`--emit-cmd` 直接给出 dump 文件清单 | **微批流水线的选题核心**，见附录 A-4 |
...
| **`AI相关/_scripts/ws_progress.py`** | 🆕 **（2026-09-04）进度数字的唯一来源**：已学习（索引 vs 账本双口径，不一致会告警）、候选池与 state 分布、待学习、无 Lua、完整性 pending / 游标页、语料库文件数。`--fast` 不重建索引，`--json` 出机器可读 | **每轮开场与收尾各跑一次**，报告直接引用 |
...
**🔴 重复 ≠ 可以跳过**（2026-09-04 实测踩坑）

把「内容重复」一律当成「学过了、跳过」，会**永久丢掉知识**：主语料里躺着某段脚本，
不代表它已经变成例题——可能只是提取过、还没轮到学。

实测：`2628490430` 本批 28 个唯一脚本全部命中主语料，其中 26 段归属作品已学习（真能跳过），
但 **`2628490430_vehicle_2.lua`、`2628490430_vehicle_0.lua` 两段归属作品 `2628490430` / `2931581433`
都还没出例题**——按旧逻辑全跳，这两段就再也不会被学。

因此 `diff_lua_batch.py` 现在会按 `ws_reference` 已学习索引把重复项拆成两类：
...
判定口径：主语料文件名首段即工坊 id，查 `_workshop_learned.json` 的 `learned` 字段。
**只有第一类能跳过**；第二类挑 1~3 段代表性的写例题即可，不必全读。
...
## 附录 A-4 — 上下文预算与微批流水线（2026-09-04 建立）

> **问题**：旧流程是「取 8~15 个 id → 全量 `census` + 全量 `dedup` → 精读 TOP 20~80 段」。
> 三个浪费：① 全量普查的输出被 2400+ 历史语料占满，本批新作品排不上号；
> ② 与主语料重复的脚本（工坊重复率 55%+）被重读一遍；
> ③ 长脚本全文 dump，一个 7.6K 字符的文件吃掉整批预算。
> 结果：11 轮才学 103 个作品（≈9.4 个/轮）。
>
> **对策**：**批次隔离 + 差分剪枝 + 字符预算装箱 + 微批循环**。目标 24 个/轮。

### 四条硬规矩

| # | 规矩 | 为什么 |
| --- | --- | --- |
| 1 | **批次隔离**：本批提取到独立目录 `_提取暂存/_bN`，`census/dedup/diff/peek/dump` 全部带 `--src/--dir _bN` | 选题只在本批内排序，不被历史语料稀释；输出行数随批次规模缩放 |
| 2 | **先差分再精读**：`diff_lua_batch.py --dir _bN` 剔除与主语料逐字节相同的脚本，**只精读「全新」清单** | 重复代码不重复学，是吞吐提升最大的一环 |
| 3 | **字符预算装箱**：用 `peek_lua.py --budget N` 让脚本按分数挑出「预算内最多文件」，而不是人肉挑 TOP 20~80 | 单个文件默认不超过预算 1/5，防止巨无霸吃掉整批；固定上下文内文件数最多 |
| 4 | **微批循环 + 即时落盘**：一轮 24 个 id 切成 3 个微批（每批 8~10），每批走完「提取→差分→装箱→精读→写例题→并入主库」再开下一批 | 单批上下文可控；中途超时/限流，已学内容已落盘，不白跑 |

### 单微批命令流（照抄即可）

```bash
PY="<PY>"; S="<WS>/AI相关/_scripts"; B="<WS>/AI相关/_提取暂存/_b1"
W="<SW_WORKSHOP>"; SC="<WS>/_tools/steamcmd/.../573090"     # 两个源都要扫

# ① 批次隔离提取（--only 指定本批 id；两个源各跑一次，输出到同一批次目录）
"$PY" "$S/extract_workshop_lua.py" --src "$W" --only "$IDS" --out "$B"
"$PY" "$S/extract_workshop_lua.py" --src "$SC" --only "$IDS" --out "$B"

# ② 与主语料差分 → 产出三段清单（先跑它，再决定要不要读）
"$PY" "$S/diff_lua_batch.py" --dir _b1
#    ① 全新（优先精读）
#    ② 重复 · 归属作品已写成例题 → 可安全跳过，不占精读预算
#    ③ ⚠ 重复 · 归属作品尚未写成例题 → 必须补学（挑 1~3 段代表性的写例题）
#    🚨 ①③ 同时为空 → 整批无需精读，直接并入主库记为「重复跳过」，跳过后续步骤
#    （②③ 的区分由脚本按 ws_reference 已学习索引自动判定，不要人肉猜）

# ③ 批内普查（--quiet 省上下文）+ 去重
"$PY" "$S/census_lua.py" --src "$B" --quiet
"$PY" "$S/dedup_pick.py" --src "$B" --topn 12 --notopic

# ④ 字符预算装箱 → 拿到 dump 清单（--pick 只吃 ①+③，别把 ② 塞进来）
"$PY" "$S/peek_lua.py" --src _b1 --pick "$PICK" --budget 22000 --emit-cmd

# ⑤ 精读（--maxc 截断保护）
"$PY" "$S/dump_lua.py" --dir _b1 --maxc 4000 <清单文件…>

# ⑥ 写例题 → 落盘 → 并入主语料 → 下一微批
mv "$B"/*.lua "<WS>/AI相关/_提取暂存/"
```

### 预算建议（按自动化单轮 ~1 小时）

- 每微批精读预算 **22 000 字符**，单文件上限 **4 000 字符**（超了用 `--maxc` 截断后摘核心）。
- 一轮 **3 个微批**，整轮精读总量 **≤ 70 000 字符**；这是上下文的安全线，不要突破。
- 若某微批的「全新」清单本身就超过预算（大作品 100+ 块），只吃装箱结果，**剩下的留到下一轮**——宁可多轮，不要爆上下文。

### 报告口径（每轮收尾必跑 `ws_progress.py`）

- **已学习** = 例题正文带「`- 来源：steam id <id>`」的作品数（`ws_reference` 索引 `learned=true`）。
- **待学习** = 候选池中 state ∈ {新, 已提取, 已暂存, 已订阅} 的数量（已学习、无 Lua 不算）。
- **未学习** = 待学习 + 无 Lua（后者是已确认无货，不再是待办）。
- 索引口径与账本 `ids_learned` 不一致时，**以索引为准**并在收尾同步账本。
