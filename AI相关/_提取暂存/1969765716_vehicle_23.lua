-- source: steam id 1969765716 / vehicle.xml block#23
...
function onTick()
	rx=64
	ry=64
	inputX=ip.getNumber(23)
	inputY=ip.getNumber(24)
	pressed=ip.getBool(12) and not isPressed--pulse
	isPressed=ip.getBool(12)

	gpsx=ip.getNumber(15)
	gpsy=ip.getNumber(16)

	compass=ip.getNumber(20)*pi2*-1
	alt=ip.getNumber(8)
	if alt<0 then alt=0 end

	zoomingIn=isPressed and iPIR(inputX,inputY,0,ry/5+15,6,ry/5)
	zoomingOut=isPressed and iPIR(inputX,inputY,0,ry/5*2+15,6,ry/5)
	reset=isPressed and iPIR(inputX,inputY,0,ry/5*3+15,6,ry/5)

	if reset then focusZone=false zoom=1.5 grid=250 end
	if isPressed and inputX>6 and not iPIR(inputX,inputY,56,0,8,8) then focusZone=true end
	if focusZone then
		if pressed and inputX>6 and not iPIR(inputX,inputY,56,0,8,8) then tx,ty=map.screenToMap(mx,my,zoom,rx,ry,inputX,inputY) end
...
function onDraw()
	...
	s.setMapColorOcean(10,10,10)
	s.setMapColorShallows(15,15,15)
	s.setMapColorLand(50,50,50)
	s.setMapColorGrass(70,70,70)
	s.setMapColorSand(60,60,60)
	s.setMapColorSnow(60,60,60)
	...
	s.setColor(0,0,0,150)
	s.drawRectF(0,h/5+15,6,h/5*3)

	if zoomingIn then s.setColor(255,255,255) else s.setColor(255,255,255,20)end
	s.drawTextBox(1,h/5+15,4,h/5,"+",0,0)
	if zoomingOut then s.setColor(255,255,255) else s.setColor(255,255,255,20)end
	s.drawTextBox(1,h/5*2+15,4,h/5,"-",0,0)
	if reset then s.setColor(255,255,255) else s.setColor(255,255,255,20)end
	s.drawTextBox(1,h/5*3+15,4,h/5,"r",0,0)
