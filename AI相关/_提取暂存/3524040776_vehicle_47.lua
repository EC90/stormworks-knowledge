-- source: steam id 3524040776 / vehicle.xml block#47
-- url: https://steamcommunity.com/sharedfiles/filedetails/?id=3524040776
--PYO's FCS TRT v20250505 MACIWS
GN=input.getNumber
...
ar=Math.sqrt
P2=Math.pi*2
--no fire area symmetry
--start,end,minele,maxele
nfa={
	{0,30,-90,20},
	{135,180,-90,70}
}
...
function Avrg(tbl,value,time)
	time=Math.max(MathFlr(time+0.5),1)
...
function onTick()
	...
	aspit=GN(32)
	--from CMD
	TGT1={GN(11),GN(12),GN(13)}
	TGT2={GN(14),GN(15),GN(16)}
	if TGT1[1]~=0 then
		if MathDist(TGT1,TGT2)<MathDist(TGT1,GunPos)*0.1 then
			TGTpos={(TGT1[1]+TGT2[1])/2,(TGT1[2]+TGT2[2])/2,(TGT1[3]+TGT2[3])/2}
		else
			TGTpos=TGT1
		end
		if TGTposO[1]~=0 then
			TGTv[1]=Avrg(t1,TGTpos[1]-TGTposO[1],30)
			TGTv[2]=Avrg(t2,TGTpos[2]-TGTposO[2],30)
			TGTv[3]=Avrg(t3,TGTpos[3]-TGTposO[3],30)
		end
		mode=3
	else
		TGTpos={0,0,0}
	...
	EleMax=PN('+')/360
	--drop and eta cal
	if TGTpos[1]~=0 and TrtReset>0.5 and not reload then
	...
	conB=isxlimit and Cps2Delta(aimPit,GunCps)<TrtRot-0.49
	conC=isxlimit and Cps2Delta(aimPit,GunCps)>TrtRot+0.49
	if conA or conB or conC then
		TrtReset=0
	end
	if TGTpos[1]==0 or TrtReset<0.5 then
		rstX=Pid(1,TrtRot*P2,0,PN('pivot x p'),PN('pivot x i'),PN('pivot x d'),PN('pivot x p m'),PN('pivot x i m'),PN('pivot x d m'))
		--prevent spike
		rstY=0
	else
		...
		Aiming=conA or conB or not ARM
	end
	--x limit
	if isxlimit then
	...
	--no fire area check
	clear=true
	cosCurX=MathCos(TrtRot*P2)
	sinCurY=MathSin(rstY*P2/4)
	for i=1,#nfa do
		cosMinX=MathCos((nfa[i][1]/360)*P2)
		cosMaxX=MathCos((nfa[i][2]/360)*P2)
		sinMinY=MathSin((nfa[i][3]/360)*P2)
		sinMaxY=MathSin((nfa[i][4]/360)*P2)
		if cosCurX<cosMinX and cosCurX>cosMaxX and sinCurY>sinMinY and sinCurY<sinMaxY then
			clear=false
			break
		end
	end
	--trigger
	SB(1,MathDist(TGTpos,GunPos)<property.getNumber('shoot distance') and TGTpos[1]~=0 and (not Aiming) and (TrtReset>0.5) and (not reload) and clear)
	ammo=GN(30)
	if reload then rldcounter=rldcounter+1 end
	if (reload and ammo==PN('ammo rack')) or rldcounter>1200 then reload=false rldcounter=0 end
