-- source: steam id 3358095813 / vehicle.xml block#4
...
if tx>w-17 and ty<40 and(GB(1)and not tc)then
tgtt=M.ceil((ty-1)/10)
if tx>w-9 then
tgtt=tgtt+4 
end
...
if curt.m<2000 and curt.e<d2t and curt.va>1 then
...
SC(64,64,64)
DR(_-2,h-26,32,32)
N(_,h-24,'d'..SF('%4.1f',Dst(sp,curt.p)/1e3))
fmt='%4.0f'N(_,h-18,'v'..SF(fmt,curt.va*60))
N(_,h-12,'a'..SF(fmt,curt.p[3]))
N(_,h-6,'b'..SF(fmt,curt.c))
end
end
end
if tc and tx>w-20 and ty>h-26 then
slid=0
end
for e=1,8 do
if e<5 then
dcx=w-17
dcy=1+(e-1)*10 else dcx=w-8
dcy=1+(e-5)*10 
end
if slot[e][4]~=0 then
...
N(dcx1-2,dcy1+4,name[e])else SC(16,16,16)
end
if e==sltgt then
DRF(dcx,dcy,8,9)
