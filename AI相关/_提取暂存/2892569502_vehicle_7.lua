-- source: steam id 2892569502 / vehicle.xml block#7
...
ta=30
function cmpr(a,b)
return math.abs(a.a)+math.abs(a.e-ce)<math.abs(b.a)+math.abs(b.e-ce)
end
function clp(v,a,b)
...
if #Rs>0 then
if (math.abs(Rs[1].a-tgtao)>0.005 or math.abs(Rs[1].e-tgteo)>0.005 or math.abs(Rs[1].d-tgtdo)>25) and locked then
locked=false
end
tgta=Rs[1].a
tgte=Rs[1].e
...
screen.drawRect(w/2-se*rdxa/fovt,h/2-se*(rdya-ce)/fovt,2*se*rdxa/fovt,2*se*rdya/fovt)
end
if #Rs>0 then
for i=1,#Rs do
x=w/2+se*(Rs[i].aa/(0.5*fovt))
y=h/2-se*((Rs[i].ae-ce)/(0.5*fovt))
if i==1 then
aimx=w/2+se*(aima/(0.5*fovt))
aimy=h/2-se*(aime/(0.5*fovt))
screen.setColor(200,15,15,190)
