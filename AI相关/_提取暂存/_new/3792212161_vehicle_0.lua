-- source: steam id 3792212161 / vehicle.xml block#0
-- url: https://steamcommunity.com/sharedfiles/filedetails/?id=3792212161
Data={}
Volumes={}
debug=true
iB=input.getBool
...
min_clutch=math.max(idle-.25,clutch_rps-1)
function onTick()
	ign=iB(1)
	ign_pulse=iB(2)
	air_volume=iN(1)
	...
	if th_mode~=0 then
		if gen_mode and th_lever==0 then
	...
	notn=false
	if th_lever==0 then n=true else if n then notn=true end n=false end
	...
	clutch=PID(rps,clutch_rps,.02,.002,.05,0,1,toggle and rps>min_clutch and th_lever>0 and not notn,"clutch")
	gen_clutch=PID(rps,clutch_rps,.02,.002,.05,0,1,toggle and rps>min_clutch and gen_mode and not notn,"gen_clutch")
	if th_lever>0 then gen_clutch=(1-battery+.1)*gen_clutch end
