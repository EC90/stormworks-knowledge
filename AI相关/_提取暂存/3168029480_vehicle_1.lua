-- source: steam id 3168029480 / vehicle.xml block#1
...
if #U>0 then
for i=1,#U do
if U[i].l>10 then T.remove(U,i) break end
end
end
if #U>0 then
for i=1,#U do
...
E=M.min(#U[i].dB-1,Mf(Mp(8*AT/U[i].v,0.25*AT,AT)))
if #U[i].dB>2 then
_x,_y,_z=0,0,0
_X,_Y,_Z=0,0,0
...
_y=_y+U[i].dB[j][2]
_z=_z+U[i].dB[j][3]
_X=_X+U[i].dB[j][1]-U[i].dB[j-1][1]
_Y=_Y+U[i].dB[j][2]-U[i].dB[j-1][2]
_Z=_Z+U[i].dB[j][3]-U[i].dB[j-1][3]
end
U[i].vx,U[i].vy,U[i].vz=_X/E,_Y/E,_Z/E
U[i].v=Mr(U[i].vx^2+U[i].vy^2+U[i].vz^2)*60
U[i][1],U[i][2],U[i][3]=_x/E,_y/E,_z/E
end
U[i].d=Dst(U[i],sp)
U[i].a,U[i].e=L2AE(G2L(U[i]))
end
U[i].l=U[i].l+1
end
end
...
for i=1,#W do
Jr,Kr=W[i].a,W[i].e
J,K=Mf(w/2+w*(Jr/(fov*w/h))),Mf(h/2-h*(Kr/fov))
SC(22,222,22)
DST(fx+J,fy+K,1)
if (L==0 and i==1) or W[i].id==L then
DT(fx+J-6,fy+K+5,SF('%2.1f',W[i].d/1000))
DT(fx+J-6,fy+K-11,SF('%3.0f',Mf(W[i].v*3.6/50)*50))
end
if W[i].id==L then
if cnt>60 then SC(222,22,22) else SC(22,222,22) end
DST(fx+J,fy+K,0)
end
end
end
