-- source: steam id 3261800786 / vehicle.xml block#0
...
function onTick()
    ...
    ad=GN(1)
	kv=sgn(vz+0.5)*(1-Mp(Ma(vz)/50,0,0.75))
	...
	if property.getBool('Twin Flow Trans') then
		...
		if vz*oL<-2 or (sgn(ws)*oL<0.2 and Ma(vz)<5) then
			...
			cL=Mp(sgn(ws)*oL*0.75,0,0.8)
			bL=false
		end
		if vz*oR<-2 or (sgn(ws)*oR<0.2 and Ma(vz)<5) then
			cR=0 bR=true
		else
			cR=Mp(sgn(ws)*oR*0.75,0,0.8)
			bR=false
		end
	end
	...
	if sys then
		if GB(24) then
			...
			_=8+12*M.max(cL,cR)+7*(1-cnt2/cnt3)*M.max(cL,cR)+(1-M.max(cL,cR))*(1-btr)*16
		end
	...
	--gear
	for i=1,#Rs do
		if Ma(vz)>G1s*(Rs[i]/Rs[1]) and gear==i and gear<#Gs then gear=i+1 cnt1=30 cnt2=tpg*gear cnt3=tpg*gear
