-- source: steam id 2372828168 / vehicle.xml block#14
...
function onDraw()
	w = 32				  -- Get the screen's width and height
	h = 32					
	
	screen.setColor(200, 200, 200, 255)
	
	--mini airplane
	screen.drawLine(64/2,94/2-10,64/2,94/2+10)
	AHand(0,9,5)
	screen.drawLine(64/2 -3,94/2+7,64/2 +4,94/2+7)
	screen.drawLine(64/2 -10,94/2,64/2 +10,94/2)
	...
	screen.drawRectF(64/2 -9, 94/2 -4, 18, 9)
	screen.setColor(0, 0, 0, 255)
	screen.drawRectF(64/2 -8, 94/2 -3, 16, 7)
	screen.setColor(200, 200, 200, 255)
	screen.drawTextBox(64/2 -8, 94/2 -2, 16, 5, string.format("%03.0f", HDG), 0, -1)
...
function rMark(deg, length)
	screen.drawLine(w/2 + 16 - w/2 * math.sin(deg), h/2 + 32 - h/2 * math.cos(deg), w/2 + 16 - (w/2 - length) * math.sin(deg), h/2 + 32 - (h/2 - length) * math.cos(deg))
end
...
function AHand(ALT,length,width)
	...
	width = width/2
	screen.drawTriangleF(64/2 + length * math.sin(deg),
		94/2 - length * math.cos(deg),
		64/2 - width * math.cos(deg),
		94/2 - width * math.sin(deg),
		64/2 + width * math.cos(deg),
		94/2 + width * math.sin(deg))
end