-- source: steam id 3464467373 / vehicle.xml block#6
...
aimgps=TB(w/2-4+ss*(w/2-13),h-6-ss*(h-7),15+ss,6+ss,aimgps)
ntmd=TB(ss,ss,8+ss*8,5+2*ss,ntmd)
share=TB(2*ss+6,h-6-2*ss,5+ss,6+ss,share)
SB(31,share)
if not SS then
--map offset
...
--zizo
if PB(w-4-2*ss,h/2-5,4+ss,6+ss) then zi=true z=math.max(z*0.97,0) end
if PB(w-4-2*ss,3*h/4-5,4+ss,6+ss) then zo=true z=math.min(z*1.03,50) end
if PB(1,h-8,6,7) then mapofst=false mox,moy=0,0 end
end--a
SN(1,et)
SB(1,ntmd)
SB(32,aimgps)
...
SN(31,mcy+moy)
SN(32,z)
if PB(7*ss,6+ss,w-4-9*ss,h-12-3*ss) and not touched and not help then
...
DB(w/2-4+ss*(w/2-13),h-6-ss*(h-7),15+ss,6+ss,"AIM",aimgps)
DB(2*ss+6,h-6-2*ss,5+ss,6+ss,"M",share)
DB(w-4-2*ss,h/2-5,4+ss,6+ss,"+",zi)
DB(w-4-2*ss,3*h/4-5,4+ss,6+ss,"-",zo)
if mapofst then
DB(ss,h-6-2*ss,5+ss,6+ss,"R",not mapofst)
end
end--a
if not SS then
