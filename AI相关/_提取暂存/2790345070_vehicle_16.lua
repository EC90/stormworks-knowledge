-- source: steam id 2790345070 / vehicle.xml block#16
...
function onTick() 
	...
	windDirection = ((windDirectionBrut-compassDirection))*2*3.14
end
...
function onDraw()
	...
	if width == 160 and height == 96 then
		...
		screen.drawLine(3*width/4,height/2,(math.cos(windDirection))*(height/4)+(3*width/4),(math.sin(windDirection))*(height/4)+(height/2))
	end
	...
	if width == 64 and height == 64 then
		...
		if valueOrWD == 0 then
			...
			screen.drawRectF(0, 0, width, 7)
			screen.setColor(100,100,100)
			screen.drawText(16,1,'Weather')
			screen.setColor(policeRed,policeGreen,policeBlue)
			screen.drawText(1,2*height/8,'Temp:' .. string.format("%.0fC", temp))
			screen.drawText(1,3*height/8,'Rain:' .. calculateMeteoRain())
			if unit == 1 then
				windSpeed = math.floor(windSpeed/1.944*100)/100
				unitString = 'm/s'
