-- source: steam id 3464467373 / vehicle.xml block#4
...
function onDraw()
	...
	if cps~=0 then
	screen.drawLine(selfx,selfy,selfx-0.4*w*math.sin((cps-rdrx*0.5)*2*pi),selfy-0.4*w*math.cos((cps-rdrx*0.5)*2*pi))
	screen.drawLine(selfx,selfy,selfx-0.4*w*math.sin((cps+rdrx*0.5)*2*pi),selfy-0.4*w*math.cos((cps+rdrx*0.5)*2*pi))
	end
	sC(15,233,15,200)
	screen.drawCircleF(Mf(selfx),Mf(selfy),0.9)
	sC(0,0,0,128)
	screen.drawCircle(Mf(selfx),Mf(selfy),0.9)
	--show target pos
	for i=1,#rstd do
		if rstd[i]>100 then
