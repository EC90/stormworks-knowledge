-- source: steam id 3524040776 / vehicle.xml block#62
...
function onTick()
	...
	--drop and eta cal
	ARM=true
	if (lsr or mode>2.5) and TrtReset>0.5 then
		if mode>2.5 then
		...
		if ARM then
			...
			for w=1,4 do
				...
				for SimStep=1,ShellPara.lf do
					for xyz=1,3 do
						ShellPos[xyz]=ShellPos[xyz]+Shellv0xyz[xyz]/62
	...
	SN(8,GN(27))
	--trigger
	SB(1,GN(25)>0.5 and (not Aiming) and (TrtReset>0.5))-- and (not reload)) 
	ammo=GN(30)
