-- source: steam id 3584922070 / vehicle.xml block#21
...
width=pN("Width")
max_width=pN("MaxWidth")
curvature_gain=pN("CurvatureGain")

function onTick()
	ad=GN(8)
	speed=GN(7)
	max_length=dis({GN(1)-GN(4),GN(2)-GN(5),GN(3)-GN(6)})
	length=0
	max_Gforce=GN(11)*10
	...
	if -turning_center > max_length/2 then
		curvature_scale=abs(-turning_center+max_width/2)
	else
		curvature_scale=abs(turning_center+max_length+max_width/2)
	end
	
	max_curvature_wheelbase=1/curvature_scale
	max_curvature_gforce=max_Gforce/(speed^2+0.0001)
