-- source: steam id 3378671427 / vehicle.xml block#5
...
function onTick()
	...
	if #TGTS>0 then
		if GN(28)>9999 then AimID=TGTS[1].id end
	...
	--1=0.25turn
	--ViewX=(MathFlr(fromPVT/1e4)-500)/2000
	ViewX=0
	ViewY=(MathFlr(fromPVT%1e4/10)-500)/2000
end
Scr=screen
SetC=Scr.setColor
	...
	if #TGTS>0 then
		for i=1,#TGTS do
			...
			if AimID==0 and i==1 or TGTS[i].id==AimID then
				...
				--DrawText(drawX-6,drawY-17,string.format('%3.0f',TGTS[i].spl))
			end
