...
> 变体注记（2026-09-07）：来源：steam id **3796370321** 的 `_vehicle_0`（电气负载表）是本节的入门版——正向 `drawRectF(x, y, load/100*宽, 4)` 填充 + `drawTextBox(0, -4, w, h, "JB", 0, 0)` 负 y 偏移画大字标题 + `string.format("%.0f")` 数值随条显示；同作品另 3 块（`_v1` 四格读数面板 / `_v2` `_v3` 单字母标识）同为新手级小仪表，无新手法，不另立例题。
