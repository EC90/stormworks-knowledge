...
> 变体注记（2026-09-07）：来源：steam id **3796954895**（Cyberpunk 2077 - Porsche 911 Turbo，`_vehicle_0`）走另一极端——**不用字体表，`drawLine` 逐笔画直接手写字形**（车头 "PORSCHE" 徽标，7 个字母约 20 条线段；字母 O 用**负宽 `drawRect(x,y,-w,-h)`** 画闭环）。适合一次性、字符数极少的商标/标识：省掉整张字模表，代价是每个字母都要人肉摆坐标。
...
> 变体注记（2026-09-07）：来源：steam id **3796370321** 的 `_vehicle_0`（电气负载表）是本节的入门版——正向 `drawRectF(x, y, load/100*宽, 4)` 填充 + `drawTextBox(0, -4, w, h, "JB", 0, 0)` 负 y 偏移画大字标题 + `string.format("%.0f")` 数值随条显示；同作品另 3 块（`_v1` 四格读数面板 / `_v2` `_v3` 单字母标识）同为新手级小仪表，无新手法，不另立例题。另有逐块同码副本：steam id **3796368745**（4 块与 `_v0`~`_v3` 一一对应相同））、steam id **3794971822**（4 块同样逐块对应相同）。
...
---
- 变体注记（2026-09-07 夜轨补）：来源：steam id **3450970417** 的 `_vehicle_0`（挡位/电池标签屏：P/R/D/数值四态 + `%.0f%%` 电量）与 `_vehicle_1`（装饰边框标签屏，`w<40` 时缩写 EQUIP→防溢出）同为新手级小仪表，无新手法，不另立例题。
- 变体注记（2026-09-07 夜轨补）：来源：steam id **3791112296** `_vehicle_0`（十字准星/标尺装饰屏，纯 onDraw 无逻辑）；来源：steam id **3790692767** `_vehicle_0`（手写四舍五入 `round`：正数 floor(x+0.5)、负数 ceil(x-0.5)，= `04` §1 通用工具，直接用 `//` 或现成 round 即可）。
- 变体注记（2026-09-07 夜轨补）：来源：steam id **2647654136**（单块，2022 旧作）——`string.format("%03.0fHDG", heading)` 等**带单位后缀的定宽格式化直出**五遥测标签屏，最简遥测模板。
- **不要标 learned**。已确认 `_vehicle_12` 摄像头云台菜单与 `05` §29（来源 3791924563）同族；但 `_vehicle_0`（73 分 engine 7825c）、`_vehicle_20`（64 分 engine 8266c）等引擎块未读。55 块为 minified 态。建议晨间/下轮：unminify 后按 census 挑引擎块专学（对标 `09` 册 ECU 系列查重）。
- 变体注记（2026-09-07 夜轨补）：来源：steam id **3765367132**（帆船，`_v0` 负载条与 3796370321 同款、`_v3` 遥测标签屏带单位直出，`×2.236936` m/s→mph）；来源：steam id **3783587187** 的 `_vehicle_3` 为 DVD 屏保式弹跳框（onDraw 帧驱动 `dx/dy` 边界取反，无 onTick 逻辑）、`_v5` trivial 转发（该作品 `_v0`~`_v2` 与已学 3796799293 逐块同码）。
- 变体注记（夜轨补）：来源：steam id **3795506809**（`_v0` 装饰圆环，trivial）；来源：steam id **3796215625**（`_v2` hud,engine 仪表屏族）。
- 来源：steam id **3796407129**、**3795744231**、**3795670387**、**3795705637**（块均已在主语料，晨间核对归属）。
...
---
- 变体注记（夜轨补）：来源：steam id **1891002603**（`_vehicle_6` 57 分：**圆形命中区按钮表** `Button={x,y,r}`——十字键/加减全用圆判据（相对 §4 矩形 isPointInRectangle 的圆形版，触控命中 `(dx^2+dy^2)^0.5<r`）+ 缩放平移地图）。

## §5 图形原语（BKN `docs/Screen/`）

### 5.1 扇面 / 圆弧 / 射线

> 变体注记（2026-09-07 补）：来源：steam id **3796112150** 的 `_vehicle_2`（气象屏第 2 页）用同一批原语手绘天气图标——云/雪/太阳全部由十几条 `drawLine` 逐线拼出、按 `input.getBool` 选图；同作品另 5 块与 `3796794230`/`3796902721` 逐块同码（已由 `09` §36 关联覆盖）。
...
---
- 变体注记（2026-09-07 夜轨清欠）：来源：steam id **3782284252**（`_vehicle_0`，圆类绘制小工具屏）。
- 同族载体三连（2026-09-07 夜轨补）：天气/GUI 组件包现有三个工坊载体——3784941999、3784941656、**3787489961**（`_vehicle_15` 属性签名 `GUI X Offset`/rain/humidity 逐行相同），并入时合成一条注记（3 id）。
- 同族注记（2026-09-07 夜轨补）：来源：steam id **3175324557** · <https://steamcommunity.com/sharedfiles/filedetails/?id=3175324557> · **载具**（`_vehicle_0`）——`drawArc(...)` 可变参通用圆弧（repeat-until 步进默认 22.5°，`pie=true` 切 drawTriangleF 扇形/否则 drawLine 弧线，**起止角自动交换** `if a2<a1 then swap`）；声纳数据按 -50..50 **整数方位扇区表** `Data[i]` 存储（= `03` §36 固定方位思想的表化），`(i/200)*2π` 扇区号转弧度。
- 同族进阶注记（2026-09-07 夜轨补）：来源：steam id **3784941999** · <https://steamcommunity.com/sharedfiles/filedetails/?id=3784941999> · **载具**（2.3 MB 巨作，325 块中 290 全新但高度自重复，`_vehicle_3` 为代表）——雨量七档分级天气图标（rain 0.01→1：太阳/少云/多云/阴/小雨/中雨/雷暴，雨丝 `drawLine` 成对、闪电 `drawTriangleF`×2）；**湿度→透明度线性映射** `(humidity-0.1)*283` 的半透明雾化叠层、`100/cd` 云朵亮度随雨强加深。整套 GUI 位置/边框/亮度全走 property。
- 变体注记：来源：steam id **3796879752**（Universal Monitor Auto-Scaler——onDraw 即时读屏宽高做全元素等比缩放的适配层）；来源：steam id **3796692724**（多输入小仪表）。
...
---
- 同族注记（2026-09-07 夜轨补）：来源：steam id **3345089549** · <https://steamcommunity.com/sharedfiles/filedetails/?id=3345089549> · **载具**（20+ 块，`_vehicle_6` 为本节 §7.1 仪表的镜像变体：数字框在左/右由 `Pos` 切换，`ang2=(V*0.00094)*i` 反向扇形；round/hex2rgb/放射线渐变与 §7.1 逐行同构）——同作者组件包的另一载体。
- 同族载体三（夜轨补）：来源：steam id **2992524919**（`_vehicle_8` 89 分——round/hex2rgb/属性集与 §7.1 逐行同款的双输入版；`_v5` 为小变体）——与 3345089549、2232448349 同包，仪表族载体累计 3。
- 同族载体四（夜轨补）：来源：steam id **2263356384**（100+ 块，`_vehicle_24` 88 分×6 副本——round/hex2rgb 属性集同款；仪表族载体累计 4：2232448349、3345089549、2992524919、2263356384）。
- 同族载体五（夜轨补）：来源：steam id **2554590347**（`_v5` 93 分×2、`_v4` 88 分×3——round/hex2rgb 同款，仪表族载体累计 5）。
- 同族注记（夜轨补）：来源：steam id **2078484946** 与 **2230445776**（两 id 同码：`_vehicle_42` 72 分×3 autopilot,radar,sensor——晨间核对后择近挂）。
- 同族载体六（夜轨补）：来源：steam id **2079692202**、**2085890645**、**2359829286**（三 id `_v15` 等 88 分×8 逐字节同码——round/hex2rgb 仪表族，载体累计 6）。
- 同族载体七（夜轨补）：来源：steam id **1990885175**（`_v0` 88 分×5——round/hex2rgb 同款仪表族，载体累计 7：2232448349、3345089549、2992524919、2263356384、2554590347、2079692202 系、1990885175——该组件包为工坊最流行仪表组件）。
...
---
- 变体注记（2026-09-07 夜轨补）：来源：steam id **3701150438** · <https://steamcommunity.com/sharedfiles/filedetails/?id=3701150438> · **载具**（20 块雷达/地图族，`_vehicle_13` 为代表）——**单色系梯度地图配色**：只给一个基色 `BR,BG,BB`（属性或输入），6 级 `setMapColor*` 全用 `BR-5/-10/-15/-20/-25` 派生（海洋最亮、雪最暗），比手写 8 组 RGB 省大量属性与代码，切主题只改 3 个数；GREY 布尔一键夜视灰。`_v2`/`_v5` 为雷达 PPI 变体（03 §37 族特征不显著，无签名头，按本条挂）。
- 同族注记（夜轨补）：来源：steam id **2309546198**（30+ 块，`_v29` 78 分：TR/TG/BB 属性签名与 3701150438 `_v13` 同款单色系地图族；160×96 大屏偏移布局；`_v27`/`_v28` 为配套）。
...
---
- 同码追加（2026-09-07 夜轨补）：来源：steam id **3620017437** `_vehicle_4` 与 3569623233 `_vehicle_3` 逐字节相同（同作者三按钮组组件包，见该注记）；`_v0`/`_v1` 为配套小组件。
- 同族变体注记（2026-09-07 夜轨补）：来源：steam id **3569623233** 的 `_vehicle_3`（表驱动三态触控按钮组）——`btn={{t=false,p=false},...}` 一表两态：`p` 记按住、`t` 记翻转，**松手沿翻转**（`if presses[i] then p=true elseif p then p=false; t=not t end`，按住不连翻）；按钮标签 `string.sub(string.upper(property.getText("button1") or ""), 1, 8)` 三重防护（or "" 防 nil、upper 统一、sub 截 8 字符防溢出）——属性文本进屏幕前的标准预处理。同作品 `_vehicle_0`/`_v2`/`_v4` 为配套 HUD/面板，`_v1` 装饰件。
- 同族载体三（夜轨补）：来源：steam id **3795260742**（`_vehicle_0` 与 3569623233/3620017437 同作者按钮包：`string.sub(upper(getText()))` 同款 + **首帧 initialized 默认点亮 button1** 的小差异；`_v9` 为小变体）。
- 变体注记（夜轨补）：来源：steam id **3795001837**（`_v0`/`_v2` sensor 1281c 变体对）；来源：steam id **3793787190**（独立微控 `_mc_0` 2439c，本批唯一实质全新段，晨间补精读定性）；来源：steam id **3792650190**、**3795207246**、**3793807481**、**3794051253**（块均已在主语料，归属晨间核对）。
- 同族载体四（夜轨补）：来源：steam id **3797186819**（`_v0`——"Button N name" 属性签名按钮组，与 3569623233/3620017437/3795260742 同包四载体）。
- 变体注记（夜轨补）：来源：steam id **3797164644**（`_v0` engine,logic 6168c——引擎控制族，晨间核对）；来源：steam id **3797185158**（`_v1` 2493c 待核）。
...
- 同族注记（夜轨补）：来源：steam id **2412699852**（400+ 块千位巨作，`_vehicle_0` 85 分×2）——**三张并行 8 色索引表** `cr/cg/cb = {0,255,255,0,...}`（索引→RGB 的调色板数组，轨迹点按序号取色——多色轨迹免 if 链）；cirs 圆点航迹表 + zm 缩放。
...
- 变体注记（2026-09-07 夜轨补）：来源：steam id **3793742371** 的 `_vehicle_0`~`_vehicle_2`（无人机 OP STAT 遥测页）——两列固定行距标签 + 数据列 `gapDataX` 对齐，水位三态 full/half/empty 分色；无本地状态，纯显示。
...
- 更新时间：2026-09-07
- 同族早期实例注记（2026-09-07 补）：来源：steam id **2488473761**（`_vehicle_0`，Aggressive minify 压缩态源码、带 `--yyy--` 锚点）用同一套「两级字典 gsub 逆替换 + `;` 分行 + 每像素 8 位 hex RGBA」渲染整幅彩色位图（摩托图形），另有全局亮度系数（所有通道 ×0.4）与 alpha 缩放变量；解码器细节（s==0 存 false 跳过透明像素）与本节同构，反压缩可读版见 `_提取暂存/_b8/2488473761_vehicle_0.lua` 的语义还原。
...
- 同族注记（2026-09-07 夜轨补）：来源：steam id **3784941656** ——与 3784941999 同作者组件包第二载体（`_vehicle_3` 天气图标页逐字节同构，273 段全新多为自重复副本）；并入时与 _idle_b3 的 3784941999 注记合并为一条双 id。
- 同族注记：来源：steam id **3788293452**（`p={{x,y,w,h,...}}` **扁平数组位图行段表** = 本册「小图标位图行段压缩表」节同款）；来源：steam id **3044035076**（`_v11` **多国车牌渲染器**：Country 编号分支 au/cz/fr/ge/gr... 逐国字形——位图渲染族趣味应用）；来源：steam id **3582585155**（微控触控方向盘 P5/P6 分区，§20 族）。
...
## §77 七段数码管数字字体：字模 = 一个 7 位十进制数，笔画方向用 sin/cos 现场生成

- 来源：steam id **3358095813**（`_vehicle_17` 地图标注 / `_vehicle_5` 雷达 PPI 读数）
- 更新时间：2025-04-11
- 用到：`screen.drawLine`、一张 10 元素表
- 亮点：点阵字体（§1/§22）存逐像素位图、矢量折线字体（§75）存相对步进，PYO 的数字字体走第三条路——**每个数字就是 1 个 7 位十进制数，第 j 位是 2 就画第 j 笔**；笔画方向不用存，7 笔方向恰是 (j-1)×90° 的正余弦序列，现场算

```lua
num={2111211,2212122,1222122,1221212,1222221,2222221,1121122,2222222,1222222}
num[0]=2122222                       -- 0~9 十个字形 = 十个 7 位数，全字体就这两行
function N(x,y,s)
  s=tostring(s)
  for i=1,string.len(s) do
    ...
    local N=tonumber(n)
    if N then
      if N==1 then w=2 end           -- 数字 1 本身窄：步进 2px，排版更紧凑
      local startx,starty,dx,dy=x,y+4,0,0
      for j=1,7 do
        startx,starty=startx+dx*2,starty-dy*2     -- 沿矩形字格走笔（每步 2px）
        if j==5 then starty=starty-2 end          -- 中横前抬 2px
        dx,dy=M.sin(0.25*P*(j-1)),M.cos(0.25*P*(j-1))
        if string.sub(num[N],j,j)=='2' then
          DL(startx,starty,startx+dx*3,starty-dy*3)   -- 画这一笔（定长 3px）
        end
      end
    else
      S.drawText(x,y,n)              -- 非数字字符（字母/符号）回落系统字体，混排不违和
    end
    x=x+w
  end
end
...
**要点**
- 全套 0~9 字体约 120 字符（表两行 + 20 行通用走笔），比 3×5 ROM（§22 每字符 4 个 16 进制字符 + 解包逻辑）更省，线段风格与雷达屏/战术屏更协调。
- 字模位语义：`num[N]` 第 j 位为 `2` 画第 j 笔、为 `1` 跳过。CIWS 版（steam id **3378667462**）同款表仅个别位不同（个别字形笔画微调），抄哪份都行。
- 混排技巧：数字步进 4px、`1` 步进 2px、非数字回落 `drawText`——密度接近系统小字，适合地图标注、坐标读数。
- 字号固定 5×7 px；要任意缩放/字重请改用 §75 的方案。
- 完整脚本：`../../../AI相关/_提取暂存/3358095813_vehicle_17.lua`
- 关联：`05` §1（3×4/3×5 点阵）、`05` §22（属性文本 ROM + 4 向旋转）、`05` §75（矢量折线全字母表，可缩放）
